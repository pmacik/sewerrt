...
public class RayTracer {
	...
	/**
	 * Režim stínování. Může nabývat hodnot {@link RayTracer#SHADOWS_ON} nebo
	 * {@link RayTracer#SHADOWS_OFF}. Výchozí hodnota je
	 * {@link RayTracer#SHADOWS_OFF}.
	 */
	...
	public void setCamera(AbstractCamera camera) {
		this.camera = camera;
		if (camera != null) {
			canvas = new BufferedImage(camera.getScreenWidth(), camera
					.getScreenHeight(), BufferedImage.TYPE_INT_RGB);
		} else {
			canvas = null;
		}
	}
		...
		for (int j = 0; j < screenHeight; j++) {
			for (int i = 0; i < screenWidth; i++) {
				...
				if (currentPrimaryRay != null) {
					finalColor = traceRay(currentPrimaryRay, null,
							recursionDeep);
	...
	public ColorRGB traceRay(Ray ray, Vertex origin, int deep) {
		...
		// vypocet osvetleni
		ColorRGB ambient = scene.getAmbientLight(), diffuse, specular, finalColor = scene
				.getBackgroundColor();
		...
		cameraVector = new Vector3(currentVertex.getPosition(), ray.getOrigin())
				.normalized();
		...
		if (cameraVector.dot(normalVector) > 0) {
			...
			for (int lightIndex = 0; lightIndex < 8; lightIndex++) {
				...
				if (currentLight != null) {
					...
					lightVector = new Vector3(currentVertex.getPosition(),
							currentLight.getPosition());
					lightDistance = lightVector.lenght();

					// zjištění stínu
					if (shadowsMode == SHADOWS_ON) {
						currentShadowRay = new Ray(currentVertex.getPosition(),
								lightVector);
						tmpVertex = getCrossedVertex(currentShadowRay,
								currentVertex);
						if (tmpVertex != null) {
					...
					cos = (float) Math.pow(Math.min(1, Math.max(0,
							reflectVector.dot(cameraVector))), h);
					specularIntensity = cos;

					specular = currentLight.getColor().mul(specularIntensity);

					// final
					finalColor = finalColor.add(diffuse.add(specular).mul(
			...
			if (deep > 0) {
				...
				Ray reflectedRay = new Ray(currentVertex.getPosition(),
						reflectedVector);

				ColorRGB reflectedColor = traceRay(reflectedRay, currentVertex,
						deep - 1);
	 ...
	 *            Pokud se jedná o primární paprsek, má hodnotu
	 *            <code>null</code>.
	 * @return
	 */
	private Vertex getCrossedVertex(Ray ray, Vertex origin) {
		...
		for (int k = 0; k < objectCount; k++) {
			...
			if (currentVertex != null) {
				if (origin != null
						&& (currentVertex.getOwner() == origin.getOwner())) {
					continue;
				}
				currentDepth = new Vector3(ray.getOrigin(), currentVertex
						.getPosition()).lenght();
	 ...
	 * Vrací režim stínů.
	 * 
	 * @return Režim stínů. Pokud jsou stíny zapnuty, vrátí
	 *         {@link RayTracer#SHADOWS_ON}, jinak vrátí
	 *         {@link RayTracer#SHADOWS_OFF}.
	 */
	 ...
	 *         pokud jsou stíny zapnuty, jinak vrací
	 *         {@link RayTracer#SHADOWS_OFF}.
	 */
