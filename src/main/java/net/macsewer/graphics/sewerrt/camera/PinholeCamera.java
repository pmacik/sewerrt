...
public class PinholeCamera extends AbstractCamera {
	 ...
	 */
	public PinholeCamera(Point3D position, Point3D target, Vector3 up,
			float pinholeDistance, int screenWidth, int screenHeight) {
		this(position, new Vector3(position, target), up, pinholeDistance,
	 ...
	 *            Výška rastru průmětny.
	 */
	public PinholeCamera(Point3D position, Vector3 direction, Vector3 up,
			float pinholeDistance, int screenWidth, int screenHeight) {
	...
	public PinholeCamera(Point3D position, Vector3 direction, Vector3 up,
		...
		pinholeDistance = (float) (Math.sqrt(realWidth * realWidth + realHeight
				* realHeight) / (2 * Math.tan(this.fieldOfView / 2)));
		computePinhole();
	}
	...
	public PinholeCamera(Point3D position, Point3D target, Vector3 up,
		...
		this.fieldOfView = fieldOfView;
		pinholeDistance = (float) (Math.sqrt(realWidth * realWidth + realHeight
				* realHeight) / (2 * Math.tan(fieldOfView / 2)));
		computePinhole();
	}
	...
	protected Vector3 getRayDirection(int i, int j) {
		Vector3 tmp = new Vector3(getRayOrigin(screenWidth - 1 - i,
				screenHeight - 1 - j), pinhole).normalized();
		return tmp;
	}
