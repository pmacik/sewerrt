...
public class ColorRGB extends Float3 {
	...
	public static ColorRGB saturateRGB(ColorRGB color) {
		return new ColorRGB(Math.min(255, Math.max(color.x, 0)), Math.min(255,
				Math.max(color.y, 0)), Math.min(255, Math.max(color.z, 0)));
	}
	...
	public static ColorRGB getRandomColor() {
		...
		return new ColorRGB((float) Math.random() * 255,
