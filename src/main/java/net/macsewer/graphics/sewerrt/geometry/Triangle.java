...
public class Triangle {
	...
	public Triangle(Vertex v1, Vertex v2, Vertex v3) {
		...
		normal = computeNormal(v1.getPosition(), v2.getPosition(), v3
				.getPosition());
	 ...
	 *         průsečíku. Pokud průsečík neexistuje, metoda vrátí
	 *         <code>null</code>
	 */
	public Vertex crossRay(Ray ray) {
		...
		if (aa >= 0.0f && bb >= 0.0f && (aa + bb) <= 1.0f) {
			...
			Vector3 midNormal = new Vector3(computeMidPoint(
					vertexA.getNormal(), vertexB.getNormal(), vertexC
							.getNormal(), aa, bb));

			ColorRGB midColor = new ColorRGB(computeMidPoint(vertexA
					.getDiffuse(), vertexB.getDiffuse(), vertexC.getDiffuse(),
					aa, bb));
			Float3 midTextureCoordinates = new Float3(computeMidPoint(vertexA
					.getTextureCoord(), vertexB.getTextureCoord(), vertexC
					.getTextureCoord(), aa, bb));
	 ...
	 */
	private static Float3 computeMidPoint(Float3 pA, Float3 pB, Float3 pC,
			float aa, float bb) {
